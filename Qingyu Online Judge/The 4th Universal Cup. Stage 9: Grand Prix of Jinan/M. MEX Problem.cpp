...
namespace polynomial
{
    struct poly : vector<i64>
    {
        ...
        auto friend comp(poly F, poly G)
        {
            int n;
            G.resize(n);
            vpoly Q;
    ...
    Main()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (i == 0)
                ...
                Q.push_back({0, -fac[i + 1]});
        }
        auto F = poly::bostanMori(n - 1, P, Q);
        F.resize(n);
        poly G(n);
        F.erase(F.begin());
        for (int i = 0; i < n - 1; ++i)
            F[i] = F[i] * fac[i] % mod;
        for (int i = 0; i < n; ++i)
            G[i] = (i & 1 ? -1 : 1) * ifac[i] % mod;
        F = F ^ G;
        cout << "0 ";
        for (int i = 0; i < n - 1; ++i)
            cout << (F[i] * ifac[i] % mod + mod) % mod << ' ';
        cout << '\n';
    }
};
signed main()
